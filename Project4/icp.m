function [TR,TT,res] = icp(model,data,maxIter,minIter,critFun,thres)
...
%   icp version 1.5
%
%   written by Per Bergstr?m 2015-06-16
%
% Reference:
%
...
elseif nargin<6
    ...
    thres=1e-20;                     % threshold to stop icp iterations
    if nargin<5
        critFun=0;                  % critFun method, LS
        if nargin<4
            minIter=60;              % min number of icp iterations
            if nargin<3
                maxIter=200;        % max number of icp iterations
            end
        end
    end
